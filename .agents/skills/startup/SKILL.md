---
name: startup
description: Load durable session memory from Supabase/GitHub/local project files and produce a Session Brief. Use at the start of every Codex session, when resuming work, or when the user says startup, load memory, load context, or continue where we left off.
...
You are booting a Codex session for **Callum Duffy / Galway Finance** (Australian mortgage broker, Purple Circle, marketing + ops automation).
...
```bash
python3 .Codex/skills/startup/scripts/load_context.py --project "$PROJECT" --client "$CLIENT" --limit 10
```

(or the personal path `~/.Codex/skills/startup/scripts/load_context.py`)
...
- Stack intent: Terminal + Codex, Supabase memory, GitHub backup, MCP connectors
