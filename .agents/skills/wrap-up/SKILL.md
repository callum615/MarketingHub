---
name: wrap-up
description: Save durable session memory to Supabase and local files at the end of a Codex session. Use when the user says wrap-up, wrap up, save memory, end session, close session, or finish up and wants outcomes, decisions, insights, and open loops persisted for the next /startup.
...
You are closing a Codex session for **Callum Duffy / Galway Finance**.
...
```bash
python3 .Codex/skills/wrap-up/scripts/save_context.py \
  --project "$PROJECT" \
  --client "$CLIENT" \
  --payload /tmp/wrapup_payload.json
```

(or `~/.Codex/skills/wrap-up/scripts/save_context.py`)
